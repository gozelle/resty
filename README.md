...
## Features
  ...
  * [Request](https://pkg.go.dev/github.com/gozelle/resty#Request) Body can be `string`, `[]byte`, `struct`, `map`, `slice` and `io.Reader` too
  ...
  * [Response](https://pkg.go.dev/github.com/gozelle/resty#Response) object gives you more possibility
    * Access as `[]byte` array - `response.Body()` OR Access as `string` - `response.String()`
    * Know your `response.Time()` and when we `response.ReceivedAt()`
  * Automatic marshal and unmarshal for `JSON` and `XML` content type
    * Default is `JSON`, if you supply `struct/map` without header `Content-Type`
    * For auto-unmarshal, refer to -
        - Success scenario [Request.SetResult()](https://pkg.go.dev/github.com/gozelle/resty#Request.SetResult) and [Response.Result()](https://pkg.go.dev/github.com/gozelle/resty#Response.Result).
        - Error scenario [Request.SetError()](https://pkg.go.dev/github.com/gozelle/resty#Request.SetError) and [Response.Error()](https://pkg.go.dev/github.com/gozelle/resty#Response.Error).
  ...
  * Request URL [Path Params (aka URI Params)](https://pkg.go.dev/github.com/gozelle/resty#Request.SetPathParams)
  * Backoff Retry Mechanism with retry condition function [reference](retry_test.go)
  * Resty client HTTP & REST [Request](https://pkg.go.dev/github.com/gozelle/resty#Client.OnBeforeRequest) and [Response](https://pkg.go.dev/github.com/gozelle/resty#Client.OnAfterResponse) middlewares
  * `Request.SetContext` supported
  * Authorization option of `BasicAuth` and `Bearer` token
  * Set request `ContentLength` value for all request or particular request
  * Custom [Root Certificates](https://pkg.go.dev/github.com/gozelle/resty#Client.SetRootCertificate) and Client [Certificates](https://pkg.go.dev/github.com/gozelle/resty#Client.SetCertificates)
  * Download/Save HTTP response directly into File, like `curl -o` flag. See [SetOutputDirectory](https://pkg.go.dev/github.com/gozelle/resty#Client.SetOutputDirectory) & [SetOutput](https://pkg.go.dev/github.com/gozelle/resty#Request.SetOutput).
  * Cookies for your request and CookieJar support
  * SRV Record based request instead of Host URL
  * Client settings like `Timeout`, `RedirectPolicy`, `Proxy`, `TLSClientConfig`, `Transport`, etc.
  * Optionally allows GET request with payload, see [SetAllowGetMethodPayload](https://pkg.go.dev/github.com/gozelle/resty#Client.SetAllowGetMethodPayload)
  ...
  * Resty design
    ...
    * Supports `http.RoundTripper` implementation, see [SetTransport](https://pkg.go.dev/github.com/gozelle/resty#Client.SetTransport)
    * goroutine concurrent safe
    * Resty Client trace, see [Client.EnableTrace](https://pkg.go.dev/github.com/gozelle/resty#Client.EnableTrace) and [Request.EnableTrace](https://pkg.go.dev/github.com/gozelle/resty#Request.EnableTrace)
...
### Included Batteries
  ...
  * Retry Mechanism [how to use](#retries)
    * Backoff Retry
    * Conditional Retry
    * Since v2.6.0, Retry Hooks - [Client](https://pkg.go.dev/github.com/gozelle/resty#Client.AddRetryHook), [Request](https://pkg.go.dev/github.com/gozelle/resty#Request.AddRetryHook)
...
```bash
# Go Modules
require github.com/gozelle/resty v2.7.0
```
...
import "github.com/gozelle/resty"
```
...
You can optionally provide client with [custom retry conditions](https://pkg.go.dev/github.com/gozelle/resty#RetryConditionFunc):
